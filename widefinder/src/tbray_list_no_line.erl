...
collect_loopA(Main) -> collect_loopA_1(Main, dict:new(), undefined, 0).
collect_loopA_1(Main, Dict, ChunkNum, ChunkNum) ->
    print_result(Dict),
    Main ! stop;
collect_loopA_1(Main, Dict, ChunkNum, ProcessedNum) ->
    receive
        {chunk_num, ChunkNumX} -> 
            collect_loopA_1(Main, Dict, ChunkNumX, ProcessedNum);
        {key, Key} ->
            collect_loopA_1(Main, dict:update_counter(Key, 1, Dict), ChunkNum, ProcessedNum);
        seq ->
            collect_loopA_1(Main, Dict, ChunkNum, ProcessedNum + 1);
        {dict, DictX} -> 
            Dict1 = dict:merge(fun (_, V1, V2) -> V1 + V2 end, Dict, DictX),
            collect_loopA_1(Main, Dict1, ChunkNum, ProcessedNum + 1)
    end.

%spawn(fun () -> scan_chunkA(Chunk, Collector) end),    
scan_chunkA(List, Collector) ->
    case List of
        [] -> Collector ! seq;
        "GET /ongoing/When/"++[_,_,_,$x,$/,Y1,Y2,Y3,Y4,$/,M1,M2,$/,D1,D2,$/|Rest] ->
            case match_until_space_newline(Rest, []) of
                {Rest1, []} -> 
                    scan_chunkA(Rest1, Collector);
                {Rest1, Word} -> 
                    Key = list_to_binary([Y1,Y2,Y3,Y4,$/,M1,M2,$/,D1,D2,$/, Word]),
                    Collector ! {key, Key},
                    scan_chunkA(Rest1, Collector)
            end;
        [_|Rest] -> scan_chunkA(Rest, Collector)
    end.
