%% binary pattern match + bm_search
-module(tbray9j).
-compile([native]).
-export([start/1]).
...
scan_chunk_1(Bin, DataL, S, Keys, BmCtx) when S < DataL - ?KEY_OFFSET ->
    case Bin of
        <<_:S/binary,"] \"GET /ongoing/When/",_,_,_,$x,$/,Y1,Y2,Y3,Y4,$/,M1,M2,$/,D1,D2,$/,_/binary>> when
          Y1 > 47, Y1 < 58, Y2 > 47, Y2 < 58, Y3 > 47, Y3 < 58, Y4 > 47, Y4 < 58,
        M1 > 47, M1 < 58, M2 > 47, M2 < 58, D1 > 47, D2 < 58, D2 > 47, D2 < 58 ->
            case match_until_space_newline(Bin, S + ?KEY_OFFSET, DataL) of
                {true, E} ->
        ...
        <<_:S/binary,C1,C2,C3,C4,C5,C6,C7,C8,C9,C10,C11,C12,C13,C14,C15,C16,C17,C18,C19,C20,C21,_/binary>> ->
            {Found, Shift} = bm_match([C21,C20,C19,C18,C17,C16,C15,C14,C13,C12,C11,C10,C9,C8,C7,C6,C5,C4,C3,C2,C1], S + ?PAT_LEN, ?PAT, BmCtx, 1),
            %io:format("Shift! ~p~n", [{Found, Shift}]),
