...
%% return keys, update custom dict
-module(tbray9c).
...
start([FileName]) ->
    ...
    {Dict, Segs} = lists:foldl(fun ({Keys, Head, Tail}, {Dict, Segs}) ->
                                       {update_dict(Keys), [Head, Tail | Segs]}
                               end, {[], []}, Results),
...
update_dict(Keys) -> update_dict_1(lists:sort(Keys), undefined, 0, []).
update_dict_1([Key|T], Curr, Count, Acc) ->
    case Curr of
        undefined -> update_dict_1(T, Key, Count + 1, Acc);
        Key -> update_dict_1(T, Key, Count + 1, Acc);
        _ -> update_dict_1(T, Key, 0, [{Curr, Count}|Acc])
    end;
update_dict_1([], _, _, Acc) -> Acc.
...
print_result(Dict) ->
    SortedList = lists:reverse(lists:keysort(2, Dict)),
