...
find(AccTo, Bin, Tbl, Pid, Tab) ->
    find(Bin, Tbl, Pid, Tab, 0, size(Bin) - (?STRLEN) - (?DATELEN)),
    AccTo!done.

find(Bin, Tbl, Pid, Tab, N, S) when N < S ->
    case match_front(Bin, N + (?STRLEN) - 1, ?REVSTR, 0, Tbl) of
	{false, Shift} -> 
	    find(Bin, Tbl, Pid, Tab, N + Shift, S);
	{true, _} ->
	    N1 = N + (?STRLEN),
	    N2 = N1 + 3,
	    case Bin of
		<<_:N2/binary, "x/", _:4/binary, $/, _:2/binary, $/, _:2/binary, $/, _/binary>> ->
		    case scan_key(Bin, N1 + (?DATELEN),
				  S + (?STRLEN) + (?DATELEN))
			of
			{none, F} -> find(Bin, Tbl, Pid, Tab, F, S);
			{ok, F} ->
			    N3 = N1 + (?MATCHHEADLEN),
			    L = F - N3,
			    ...
			    find(Bin, Tbl, Pid, Tab, F, S)
		    end;
		_ -> find(Bin, Tbl, Pid, Tab, N1, S)
	    end
    end;
...
scan_key(B, N, S) when N < S ->
    case B of
	<<_:N/binary, _,$\s, _/binary>> -> {ok, N + 1};
	<<_:N/binary, $.,  _/binary>> -> {none, N};
        <<_:N/binary, $\n, _/binary>> -> {none, N};
        _ -> scan_key(B, N + 1, S)
    end;
