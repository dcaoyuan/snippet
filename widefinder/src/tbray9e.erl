...
start([FileName]) ->
    ...
    Segs = [process_result(wait_result(Worker), Tid) 
            || Worker <- read_file(FileName, ?BUFFER_SIZE, ProcN1, BmCtx)],
...
process_result({Keys, I, Head, Tail}, Tab) ->
    lists:foreach(fun (Key) -> update_tab(Tab, Key) end, Keys),
    {I, [Head, Tail]}.
...
scan_file([FileName, Size, I, BmCtx]) ->
    ...
    {scan_chunk([Data, BmCtx]), I, Head, Tail}.
