...
test() ->
    ...
    5 = t7([#foo{a=5, b=7}]),
    {bar,undefined,23,undefined} = t8([#foo{a=4}, #foo{b=6}]),
...
t03() ->
    V1 = #foo{},
    %io:format("~p~n", [V1.c.x = 100]),
    % Result should be: {foo, 34, "bob", {bar, 100, 23, undefined}}
    {foo, 34, "bob", {bar, 100, 23, undefined}} = V1.c.x = 100,
    V2 = #foo{a = #bar{x = #foo{b = #bar{y = 19, z=26}}}, b=hello, c = "boo"}, 
    V3 = V2.a.x.b.y = 100,
    100 = V3.a.x.b.y.
...
perfomance_test() ->
    test_native(10000000),
    test_bird(10000000).

test_native(Cycle) ->  
    statistics(wall_clock),  
    test_native_1(Cycle, #foo{}),  
    {_, Duration} = statistics(wall_clock),  
    io:format("Native Duration ~pms~n", [Duration]).  
test_native_1(0, _InitVal) ->  
    ok;  
test_native_1(N, InitVal) ->
    Bar = InitVal#foo.c,
    Bar1 =  Bar#bar{z = 100},
    InitVal#foo{c = Bar1},  
    test_native_1(N-1, InitVal).  

test_bird(Cycle) ->  
    statistics(wall_clock),  
    test_bird_1(Cycle, #foo{}),
    {_, Duration} = statistics(wall_clock),  
    io:format("Bird Duration ~pms~n", [Duration]).  
test_bird_1(0, _InitVal) ->  
    ok;  
test_bird_1(N, InitVal) ->  
    InitVal.c.x = 100,
    test_native_1(N-1, InitVal).  
